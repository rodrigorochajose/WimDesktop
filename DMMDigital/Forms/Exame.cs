...
namespace DMMDigital
{
    public partial class Exame : Form, EventReceiver
    {
        ...
        public Exame(PatientModel paciente, List<TemplateLayoutModel> listaTemplateLayout, string nomeTemplate, string nomeSessao)
        {
            InitializeComponent();

            pacienteId = paciente.id;
            labelPaciente.Text = paciente.name;
        ...
        private void ExameCarregar(object sender, EventArgs e)
        {
            ...
            filmeSelecionado = listaFilmes.First();
        }
        ...
        private void botaoImportarClique(object sender, EventArgs e)
        {
            if (filmeSelecionado.Image != null)
            {
                ...
                importarImagem.ShowDialog();
                filmeSelecionado.Image = Image.FromStream(importarImagem.OpenFile());
                filmePrincipal.Image = filmeSelecionado.Image;
                liberarFerramentas();
            }
        }
        ...
        {
            if (indiceHistorico + 1 < historicoDesenhos.Count)
            {
                indiceHistorico++;
            }
            filmePrincipal.Invalidate();
                ...
                switch (tipoAcao)
                {
                    ...
                    case 1:
                        ...
                        if (desenhoAtual != null)
                        {
                            ...
                            {
                                case Circulo c:
                                    desenhoSelecionadoParaMover = new Circulo();

                                    break;
                                case Retangulo r:
                                    desenhoSelecionadoParaMover = new Retangulo();
                                ...
                                case Seta s:
                                    ...
                                    break;
                                case DesenhoLivre ds:
                                    desenhoSelecionadoParaMover = new DesenhoLivre
                                    {
                                        pontos = (desenhoAtual as DesenhoLivre).pontos,
                                    };

                                    diferencasPontosDesenhoLivre = new List<Point>();

                                    primeiroPontoMoverDesenho = (desenhoSelecionadoParaMover as DesenhoLivre).pontos.First();
                                    List<Point> pontosDesenhoSelecionado = (desenhoSelecionadoParaMover as DesenhoLivre).pontos;

                                    for (int contador = 1; contador <= (desenhoSelecionadoParaMover as DesenhoLivre).pontos.Count - 1; contador++)
                                    {
                                        diferencasPontosDesenhoLivre.Add(new Point(pontosDesenhoSelecionado[contador].X - primeiroPontoMoverDesenho.X, pontosDesenhoSelecionado[contador].Y - primeiroPontoMoverDesenho.Y));
                                    }

                                    break;
                                case Texto t:
                                    desenhoSelecionadoParaMover = new Texto
                                    {
                                        texto = (desenhoAtual as Texto).texto,
                                        fonte = (desenhoAtual as Texto).fonte,
                                        pincel = (desenhoAtual as Texto).pincel,
                                    };

                                    break;
                            }
                            desenhoSelecionadoParaMover.posicaoInicial = new Point(desenhoAtual.posicaoInicial.X, desenhoAtual.posicaoInicial.Y);
                            desenhoSelecionadoParaMover.posicaoFinal = new Point(desenhoAtual.posicaoFinal.X, desenhoAtual.posicaoFinal.Y);

                            historicoDesenhos.Add(new List<iDesenho>(historicoDesenhos[indiceHistorico])
                            {
                                desenhoSelecionadoParaMover
                            });
                            indiceHistorico = historicoDesenhos.LastIndexOf(historicoDesenhos.Last());
                            historicoDesenhos[indiceHistorico].Remove(desenhoAtual);
                        }
                        break;
                }
        ...
        private void filmePrincipalMouseMove(object sender, MouseEventArgs e)
        {
            if (desenhar)
            {
                ...
                {
                    if (desenhoSelecionadoParaMover != null)
                    {
                        desenhoSelecionadoParaMover.posicaoInicial = new Point(e.X + primeiroPontoMoverDesenho.X - posicaoClique.X, e.Y + primeiroPontoMoverDesenho.Y - posicaoClique.Y);
                        desenhoSelecionadoParaMover.posicaoFinal = new Point(e.X + ultimoPontoMoverDesenho.X - posicaoClique.X, e.Y + ultimoPontoMoverDesenho.Y - posicaoClique.Y);

                        if (desenhoAtual is DesenhoLivre)
                        {
                            Point primeiroPonto = (desenhoSelecionadoParaMover as DesenhoLivre).pontos.First();
                            int contador = 1;

                            (desenhoSelecionadoParaMover as DesenhoLivre).pontos[0] = new Point(e.X + primeiroPontoMoverDesenho.X - posicaoClique.X, e.Y + primeiroPontoMoverDesenho.Y - posicaoClique.Y);

                            diferencasPontosDesenhoLivre.ForEach(diferencaPonto =>
                            {
                                (desenhoSelecionadoParaMover as DesenhoLivre).pontos[contador] = new Point(primeiroPonto.X + diferencaPonto.X, primeiroPonto.Y + diferencaPonto.Y);

                                if (contador + 1 < (desenhoSelecionadoParaMover as DesenhoLivre).pontos.Count)
                                {
                                    contador++;
                                }
                            });
        ...
        private void pintarFilmePrincipal(object sender, PaintEventArgs e)
        {
            ...
            if (desenhar)
            {
                if (tipoAcao == 3)
                {
                    desenhoSelecionadoParaMover?.desenharPrevia(e.Graphics, pen);
                }
                else
                {
